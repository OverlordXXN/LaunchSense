...
def main():
    ...
    # 2. Select numeric ML features and target variable
    features = [
        'goal',
        'goal_realism_score',
    ...
    logger.info("Initializing Random Forest Classifier (n_estimators=200, n_jobs=-1)...")
    ...
    model_path = os.path.join(models_dir, 'kickstarter_random_forest_model.pkl')
    
    logger.info(f"Saving trained model to {model_path}...")
    joblib.dump(model, model_path)
    
    print(f"\nModel successfully saved to: models/kickstarter_random_forest_model.pkl")
